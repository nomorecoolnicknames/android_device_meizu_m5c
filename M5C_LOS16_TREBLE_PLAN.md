...
## 9. Поправка от владельца: дерево LOS 16 уже есть, и не пустое

FACT (2026-08-28, указание владельца + проверка): синкать `lineage-16.0` с
нуля было не нужно — дерево уже стоит на сборочной машине **gunwest**
(хост `west`, 192.168.2.x, доступ через `~/.claude/skills/gunwest/scripts/gw.sh`):

```
/home/gun/m6rom16/rom            61 ГБ, ветка manifests → origin/lineage-16.0
```

Свежескачанная копия `/srv/forge/android/m5c-los16` (11 ГБ, неполная)
удалена. **Этап 0 в части «синк дерева» закрыт: дерево есть.**

### Что в этом дереве уже сделано (и это отменяет часть плана)

FACT, `ls device/meizu` и `vendor/meizu` в дереве gunwest:
```
device/meizu/{m681, m95, M6T, meizu_m6, m3_meizu_m6-common,
              meizu_mt675x-common, mt6755-common}
vendor/meizu/{m681, m95, M6T, meizu_m6}
out/target/product/meizu_m6        (то есть сборки уже шли)
```

FACT, `device/meizu/m681/BoardConfig.mk:113-125` — **vendor-раздел ровно
нашего размера уже настроен**:
```
TARGET_COPY_OUT_VENDOR := vendor
BOARD_VENDORIMAGE_FILE_SYSTEM_TYPE := ext4
BOARD_VENDORIMAGE_PARTITION_SIZE := 536870912      # 512 МиБ = наш custom
AB_OTA_UPDATER := false                            # A-only, без слотов
TARGET_KERNEL_ARCH := arm64
```
и там же — та же стратегия, к которой пришла наша разведка независимо:
«stage A ONLY: `PRODUCT_FULL_TREBLE_OVERRIDE` остаётся false,
`PRODUCT_SHIPPING_API_LEVEL` остаётся 25, VNDK не включается — он
недостижим для этого набора блобов и **не требуется** для vendor-раздела:
`PRODUCT_USE_VNDK` гейтится уровнем shipping API, а не Treble».

FACT: `device/meizu/m681/rild-mtk-hidl.rc` — **телефония через HIDL на MTK
уже решена**. Это тот самый пункт §4.6, который разведка назвала самым
дорогим. Из шапки файла видно и решение, и грабли:
- `vendor/mediatek/ril/rild/Android.mk:65` не ставит init-rc
  (`LOCAL_INIT_RC` закомментирован), а AOSP-шный rild не собирается при
  `ENABLE_VENDOR_RIL_SERVICE := true` — поэтому rc пишется руками и
  подменяется в `PRODUCT_COPY_FILES`;
- сервис — `/vendor/bin/hw/rild`, `user root` (не radio): MTK-rild трогает
  ccci-узлы и NVRAM-пути, которых uid radio не покрывает;
- `mtkmal` — плайн-клиент: он раз в секунду стучится в сокет `rild-mal` и,
  пока ему не ответят, сам не слушает `/dev/socket/mal-mfi`; поэтому libril
  обязан обслуживать этот сокет, иначе SIM и IMS не получают ни одной
  AT-команды;
- сервис `disabled` и поднимается триггером из `init.modem.rc` после
  готовности MD1 — сохраняется откат на прежний стек.

Там же лежат `DISPLAY_SHIM_CASCADE.md`, `GRAY_STRIPES_FIX.md`,
`RUNTIME_BLOCKERS_AUDIT.md` — разбор дисплейных шимов и аудит блокеров
рантайма для того же семейства.

### Следствия для плана

- §1 (разметка): подтверждается чужим рабочим примером — vendor 512 МиБ,
  A-only. Наш `custom` совпадает по размеру байт-в-байт.
- §4.6 (телефония): из «переписать libril с нуля» превращается в «перенести
  готовый рецепт m681 и подогнать под ccci/mux m5c». Это радикально дешевле.
- §6.2 (VNDK): наш вывод «VNDK недостижим и не нужен» подтверждён
  независимо тем же деревом.
- §6.6 (место): теперь ограничение не у нас, а **на gunwest — там свободно
  11 ГБ при диске 937 ГБ (99 % занято)**. Нужно решить, где вести сборку:
  чистить gunwest либо копировать дерево к нам (у нас 156 ГБ свободно, и
  копия по локальной сети быстрее пересинка).

**Правило на будущее, стоившее 11 ГБ и часа: прежде чем синкать дерево —
проверить gunwest и соседние проекты.** Родственные устройства
(m681, m95, M6T, meizu_m6) живут в одном дереве, и их наработки применимы.
