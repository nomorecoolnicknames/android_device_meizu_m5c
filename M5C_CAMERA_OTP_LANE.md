...
## 9. Прогон на железе 2026-08-17 (вечер): что заработало и один промах

Прошивал team-lead: `forge/camotp` смерджен в master (merge `07e2a164`), собрано
в основном дереве вместе с проверенными сегодня mBack и torch, ramdisk — LOS с
внесённой правкой §4.1. Образ sha256 `f695f50d30bdba5ff481c5f729ec0cdb…`,
раздел boot, загрузка до `sys.boot_completed=1`.

### 9.1 Подтверждено на железе (FACT, dmesg/logcat)

- Все восемь узлов существуют с нужными правами, например
  `crw-rw---- 1 system camera 238,0 S5K4H8_ST_OTP`. Правка §4.1 была
  обязательной: без неё было бы `root:root 0600`.
- EEPROM читается по-настоящему:
  `[S5K4H8_OTP] S5K4H8_ST_OTP LSC 1868 bytes read, first=0xff 0x00 0x02 last=0x58`.
- Вариант определяется верно:
  `[s5k4h8_resolve_variant] camera module id 8 -> variant 1, sensor id 0x4089`,
  то есть модуль этого экземпляра — **ST** (§2.4 подтверждена на железе).
- HAL перестал падать в `ERR_NO_SHADING` и реально считает шейдинг:
  `ShadingTrans_RA: [LscRaSwMain]`, `[evaluate_Shading_Ratio]`,
  `[evaluate_Shading_CCT_index] Shading CCT Idx(old,new)=(0,2), i4CCT = 6500`.

### 9.2 Промах: HAL открывал SUNWIN, а модуль ST

FACT: `[S5K4H8_OTP] S5K4H8_SUNWIN_OTP opened but no calibration was read`
(из mediaserver и из 3ATHREAD), и в логкате
`CamCalCamCal: s5k4h8_sunwinErr: 997: LayoutType= 0x5`, `998: result= 0x8fffffff`,
`237: Return ERROR ERR_NO_3A_GAIN`.

FACT (причина, из исходников): отдаваемый sensor id тут вообще ни при чём. HAL
берёт личность сенсора из **имени той записи `kdSensorList`, которую он опросил**,
а `adopt_CAMERA_HW_CheckIsAlive()` (`kd_sensorlist.c:1441`) считает сенсор
найденным при любом id, кроме `0` и `0xFFFFFFFF`, и id с записью не сверяет.
У стока четыре записи S5K4H8 обслуживаются **четырьмя разными драйверами**, и три
из них читают id вендора, видят, что он не их, и возвращают отказ. Схлопнув
четыре записи на один драйвер (§3, п.2), я потерял именно эту дискриминацию:
наш драйвер отвечал «жив» под всеми четырьмя записями, а SUNWIN — последняя из
них.

INFERENCE: тот же механизм объясняет и симптом ДО фикса. В старом списке из двух
записей на индексе 0 стоял наш главный сенсор, он отвечал «жив», и HAL называл
его `s5k5e8st` — именем своей записи 0. Одна механика на два наблюдения.

CORRECTION к §8.3: там сказано, что фикс работает за счёт того, что неверные
записи «теперь корректно не отвечают». Для записей *другого сенсора* (S5K5E8 на
главном сокете) это так, а для трёх чужих *вариантов того же сенсора* — не было
так до коммита `c99c60f9`.

### 9.3 Правка (коммит `c99c60f9` на `forge/camotp`)

Каждой из восьми записей дана своя `*_MIPI_RAW_SensorInit`-обёртка: вызов
`SensorInit` из `kdSetDriver()` — единственный момент, когда драйвер может
узнать, какую запись опрашивают, поэтому обёртка запоминает вариант в
`g_selected_variant`. Дальше `get_imgsensor_id()` и `open()` возвращают
`ERROR_SENSOR_CONNECT_FAIL`, если опрашиваемая запись не соответствует
установленному модулю. При нечитаемом или неизвестном id вендора драйвер
claim'ит только первую запись своего сенсора (OFILM / ST), чтобы неизвестная
ревизия перечислялась один раз, а не четыре.

Проверено в собранном образе: все восемь записей сохраняют стоковые id, имена и
порядок, и каждая теперь указывает на свою `SensorInit`.

Новые маркеры в dmesg при опросе чужой записи:
`probed as variant N but module is variant M, declining this entry`.
Ожидаемо, что таких строк будет несколько (по одной на каждую чужую запись,
которую HAL успеет опросить) — это норма, а не ошибка. Главный маркер успеха:
`S5K4H8_ST_OTP` (для этого экземпляра) вместо `S5K4H8_SUNWIN_OTP` в строке
`opened ...`, и исчезновение `ERR_NO_3A_GAIN`.

### 9.4 `otp apply fail` — безвредно, ничего не гейтит (ответ на вопрос lead'а)

FACT (по исходникам, не по догадке):

- `Read_CamOtpInfo_CheckSum()` в `s5k4h8mipi_Sensor.c` читает OTP **самого
  сенсора** (страница 0xF, затем 5..10) по донорской раскладке. У m5c калибровка
  лежит во внешнем EEPROM (§2.2), поэтому `module_flag` не совпадает ни с `0x01`,
  ни с `0x13`, функция возвращает `-1`. Это ожидаемо на этом модуле.
- `Otp_Calibration()` возвращает не `0x111`, и в `open()` выполняется ветка
  `else`: она делает ровно два действия — `mtk_eeprom_hw_otp_check_set("front",1)`
  и `LOG_INF("otp apply fail")`. **Раннего возврата в этой ветке нет**, `open()`
  продолжается со `spin_lock(&imgsensor_drv_lock)` и дальше как обычно.
- `mtk_eeprom_hw_otp_check_set()` (`kd_sensorlist.c:4354`) только пишет строку
  `"otp:front_err\n"` в буфер `front_otp_checksum_info` для заводского теста.
  Никакой логики она не гейтит. Строка `kd_sensorlist: ... 4356` — это её
  собственный `pr_err(__LINE__)`.

Вывод: **безвредно, трогать не надо**. Указывать чтение `module_flag` на EEPROM
было бы вредно: `Awb_Calibration_Apply()` пишет цифровые усиления в регистры
сенсора по донорской формуле, и это накладывалось бы на AWB, который ISP уже
берёт из cam_cal — двойная коррекция. Единственный косметический дефект: статус
пишется как «front», хотя речь о главной камере.

REJECTED «`otp apply fail` мешает цвету»: ветка `else` ничего не выполняет, кроме
записи строки состояния и лога.

### 9.5 Про вердикт по картинке

Владелец смотрел кадр за три минуты до прошивки этого ядра, так что «цвет всё
ещё плохой» к этой сборке не относится. Поворот 180° закрыт дорожкой camrot
(4 байта в `libcam.metadataprovider.so`, камера 0 отдаёт Orientation 270).
Ждём свежий кадр после `c99c60f9`.
