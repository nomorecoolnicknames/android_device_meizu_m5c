...
### Дозамер: 9-минутный прогон (сессия удерживалась broadcast'ами)

- FACT: 1526 NMEA-предложений GSV за ~9 мин, все `$GPGSV,1,1,0` — 0 SV.
- FACT: движок работал всю сессию: mnld писал `/data/misc/gps/`
  (BEE.BIN 748K, ARC.BIN 151K, mtkgps.dat/cache.dat 31K), в dmesg — живые
  `GPS_ioctl` (21505/21506/21515) из mnld → цифровой путь до чипа цел.
- INFERENCE: RF, скорее всего, тоже цел — GPS в MT6737M интегрирован в тот же
  CONSYS и питается тем же TCXO, что рабочие Wi-Fi/BT; но 0 SV в помещении
  формально не отличает «нет неба» от дефекта антенного тракта. Решающий
  тест — вынести аппарат к окну/на улицу.
