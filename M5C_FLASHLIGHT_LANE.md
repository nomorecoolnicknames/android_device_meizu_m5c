...
> **СТАТУС: РЕШЕНО, подтверждено на железе 2026-08-17.** Фонарик работает,
> владелец видит ровно горящий LED. Детали проверки — в разделе
> «Результат на железе» ниже. Device ID (reg 0x0C) = **0x18**.
...
* Чип на `1-0063` использует LM3643/LM3644-совместимую карту
  (регистры 0x01/0x03..0x06/0x08). Для **этого** экземпляра это подтверждено
  железом (см. «Результат на железе»); на ревизии с настоящим LM3642 новый код
  фонарь не включит — там понадобится ветвление по `readReg(0x0C)`, которое
  сейчас в лог только печатается, но ни на что не влияет.
...
#### Вариант без регрессии mBack

FACT: правка mBack лежит в ветке `forge/mback` (`55c14fdc`) и в `master`
(`d9c41e0e`) её НЕТ, поэтому чистое `forge/torch` ядро её не содержит. Собран
второй вариант на выброшенной ветке `forge/torch-mback` = `forge/torch` +
merge `forge/mback` (merge-коммит `b167179c`):

* `Image.gz-dtb` = 7 844 561 B, sha256
  `4c2fa7f14ebc6a0612676909cf81815396bb5ecb6ff26f8670bb3025fe7540c5`,
  путь `<скретчпад>/torch_mback_Image.gz-dtb`; DTB внутри тот же стоковый
  md5 `e17a091033c1e9188df898186be2761f`, `gunzip -t` пройден. Баннер
  `... #2 SMP PREEMPT Mon Aug 17 16:06:54 MSK 2026`.
* Чистый вариант только с фонариком: `<скретчпад>/torch_Image.gz-dtb`,
  7 844 933 B, sha256
  `2f09edb54f39b8f2285e510327c94ffdb03dda4d7e2b42cea0acb829cbd5f0df`
  (ветка `forge/torch`, `ba5b9145`).
...
## 2026-08-17 Результат на железе: фонарик работает

Прошивал и проверял владелец сессии (team-lead), не агент. Прошито НЕ моими
готовыми образами: `forge/mback` смерджен в `master` (merge `e56d6726`), затем
поверх смерджен `forge/torch` (merge `80fd92ed`), собрано в основном дереве и
перепаковано с текущим LOS-ramdisk; баннер `3.18.19 (root@n8nagent)`.
Ветки `forge/torch` и `forge/mback` не тронуты; временная `forge/torch-mback`
удалена как ненужная.

FACT: **LED задней вспышки светит и держится включённым** — подтверждено
владельцем визуально. Ни одного побочного эффекта: камера по-прежнему
перечисляет 1 устройство, плитка фонаря в UI переключается чисто, обрезания по
таймауту через 1 с нет.

FACT: сработали все ожидаемые маркеры:

* загрузка: `[KD_CAMERA_FLASHLIGHT]flashlight_gpio_init done, ret = 0`, далее
  `char device already created, pins only` (то есть оба platform-устройства
  прошли probe, как и задумано: пины взяты у DT-узла, chrdev создан один раз);
* **DT-узел привязан впервые за всё время**:
  `/sys/devices/bus/bus:flashlight/driver -> ../../../bus/platform/drivers/kd_camera_flashlight`.
  До патча symlink `driver` там отсутствовал — это ровно то, что было измерено
  в 15:47 и перепроверено в 16:03;
* нажатие: `[leds_strobe.c]FL_Enable: torch on, duty = 0, level = 0x23`,
  `[KD_CAMERA_FLASHLIGHT]pin(0) state(1) ret(0)`;
* путь strobeId == 2 тоже реально вызывается:
  `[strobe_main_sid2_part1.c]FL_Enable: FL_Enable, forwarding to the constant
  flashlight strobe` — то есть проброс id 2 был не перестраховкой, а
  необходимостью (стоковый шаблон здесь пустой);
* отпускание: `FL_Disable: off`, `pin(0) state(0) ret(0)`;
* readback: `[leds_strobe.c]FL_Enable: readback enable = 0x0b flag2 = 0x00
  devid = 0x18` — регистр enable читается ровно тем `0x0b`, что мы записали,
  флаги чистые.

### Идентификация чипа: что теперь FACT, а что нет

FACT: `readReg(0x0C)` = **0x18** на экземпляре `710HVBR923RYK`.

FACT: 0x0C — это именно Device ID в этом семействе: MTK-референс
`/srv/forge/android/m5c/kernel-m5c-4.9-lc/drivers/misc/mediatek/flashlight/flashlights-lm3644.c:67`
— `#define LM3644_REG_DEVICE_ID (0x0C)`, и драйвер его читает, чтобы отличить
LM3644TT.

FACT: наша регистровая последовательность совпадает с задокументированной картой
LM3643 **во всём**, что мы трогаем —
`.../flashlights-lm3643.c:46-67`: `REG_ENABLE 0x01`
(`ENABLE_LED1_TORCH 0x09`, `ENABLE_LED2_TORCH 0x0A` → оба = `0x0B`;
`ENABLE_LED1_FLASH 0x0D`, `ENABLE_LED2_FLASH 0x0E` → оба = `0x0F`),
`TORCH_LEVEL_LED1 0x05`, `TORCH_LEVEL_LED2 0x06`, `FLASH_LEVEL_LED1 0x03`,
`FLASH_LEVEL_LED2 0x04`, `TIMING_CONF 0x08` со значением
`TORCH_RAMP_TIME 0x00 | FLASH_TIMEOUT 0x0F` = `0x0F`, `FLAG1 0x0A`,
`FLAG2 0x0B`. Плюс `README_LM3643`: slave address **0x63** и «GPIO for
enable/disable chip» — оба совпадают с нашим железом.

КОРРЕКЦИЯ к первой записи: регистр `0x0B` был у меня назван `FLAG1`; по карте
семейства это **FLAG2** (`FLAG1` — это `0x0A`). Исправлено в коммите
`b400984c` (только имена и комментарии, поведение не менялось; единственное
видимое отличие — в одной строке лога `flag1` → `flag2`, учитывайте при grep'е
логов от образа, прошитого 2026-08-17).

**Честная формулировка идентификации.** Ни один артефакт, доступный на этой
машине, не связывает `0x18` с конкретным номером детали, и датасheet SY7806 у
меня нет. Поэтому правильно писать:

> регистровая карта — LM3643/LM3644-совместимая, Device ID (0x0C) = 0x18

и НЕ писать «это SY7806». Имя «SY7806» взято исключительно из имён функций
стокового драйвера (`SY7806_set_torch_mode` и т.д.) — это ярлык Meizu/Huaqin, а
не идентификация. INFERENCE: имена функций в стоке здесь ненадёжны по прямому
доказательству из этого же файла — он десять лет носил имя `LM3642` и
i2c-driver name `leds-LM3642` для чипа, чья регистровая карта на LM3642 не
похожа вообще. Ровно поэтому доверять `SY7806_*` как идентификатору нельзя.

Что могло бы это закрыть: датасheet SY7806 (или AW3644/SGM3785A) с таблицей
Device ID, либо разбор userspace-блоба вспышки, либо фото маркировки чипа на
плате.

### Не проверено / открыто

* Ничего из этого пока не проверено на железе. Пропадание adb в 15:5x было
  перезапуском adb-сервера на девбоксе, устройство вернулось; в 16:03 факт
  «`bus:flashlight` без symlink `driver`» перепроверен на **текущем** работающем
  ядре (`3.18.19 ... #2 ... 15:42:52 MSK`) — полный листинг
  `/sys/devices/bus/bus:flashlight/`: `driver_override`, `modalias`, `power`,
  `subsystem`, `uevent`. Там же подтверждено, что i2c-клиент
  `/sys/devices/bus/11008000.i2c/i2c-1/1-0063/driver -> leds-LM3642` привязан.
* Неизвестно, какой `strobeId` использует наш HAL-блоб. Патч закрывает оба.
* ЗАКРЫТО: `readReg(0x0C)` дал `0x18` на железе, см. раздел «Результат на
  железе». Осталось только сопоставить `0x18` с номером детали — нужен
  датасheet или маркировка чипа.
* Открыто: чип включается двумя каналами (`0x0B`) вслепую. Сколько LED реально
  распаяно на m5c, не проверено; `flag2 = 0x00` говорит, что защита по
  open-LED не срабатывает, но это не доказывает наличие двух LED. Если
  понадобится точный ток — читать `FLAG1` (0x0A) и `FLAG2` (0x0B) и,
  возможно, разделить каналы.
* Открыто: `duty = 0` даёт `level = 0x23` — минимальную ступень torch-таблицы.
  Более яркие ступени (`0x31`/`0x6A`/`0x7F`) на железе не проверялись, как и
  flash-режим (`duty > 3`, `0x01 = 0x0F`) для собственно съёмки со вспышкой.
  Это следующий шаг дорожки: снимок со вспышкой.
