...
## 2026-08-25 (продолжение) — звук ПОЧИНЕН; conn49 гасит USB целиком

### Звук: причина найдена точно и исправлена (FACT)

**Поправка к предыдущей записи.** Девять отказов
`AudDrv_GPIO_probe pinctrl_lookup_state ... fail -19` — **не причина**. Они
безобидны: `AudDrv_GPIO_probe` возвращает `void` и только предупреждает,
строгий вариант с ранним возвратом спрятан под `#if 0`. Стоковый узел
`audio@11220000` объявляет ровно два состояния (`extamp-pullhigh`,
`extamp-pulllow`) — остальные девять драйвер просит зря, и это норма для этой
платы.

**Настоящая причина.** Сверил `compatible` **всех 28** звуковых драйверов
`sound/soc/mediatek/mt6735/` со стоковым DTB (разобран `dtc`). Отсутствует
ровно один: **`mediatek,mt_soc_pcm_deep_buffer_dl`**. Значит его платформенное
устройство не создаётся, `snd_soc_register_card` валит карту целиком
(`ASoC: platform mt-soc-deep-buffer-dl-pcm not registered`), без карты не
стартует `audioserver`, а без него не завершается загрузка.

**Исправление** (`f69f3b0bb`, ветка `aud49-fix`): связка `DEEP_BUFFER_DL_OUTPUT`
в `mt_soc_machine.c` убрана под `#if 0` с объяснением. Одна связка — карта
поднимается на 27 платформах, которые в стоке есть.

**Проверено на железе** (`boot_49_audfix.img`, md5
`2041c49235349d01f8a1e9c61466d6bd`, гейт пройден):

- `/proc/asound/cards` → `0 [mtsndcard]: mt-snd-card`
- 30 PCM-узлов в `/dev/snd/`
- `audioserver` и `system_server` работают
- **загрузка завершается за 74 с**, тогда как до правки не завершалась вовсе
- дисплей (fb0), `/system`, `/data`, zygote — живы

Заодно закрыт способ: сверять `compatible` драйверов со стоковым DTB списком,
а не по одному. 28 против 27 нашлось одной командой.

### Сборка: цель `Image` в этом дереве тянет сломанный DTS

`arch/arm64/boot/dts/mediatek/k37mv1_bsp_k49.dts:529` включает несуществующий
`k37mv1_bsp_k49/cust.dtsi` → `make Image.gz` падает, хотя ядро линкуется. Нам
ядерный DTB не нужен (клеим стоковый), поэтому образ собирается из `vmlinux`
штатным правилом вручную:

```
aarch64-...-objcopy -O binary -R .note -R .note.gnu.build-id -R .comment -S \
        vmlinux arch/arm64/boot/Image
gzip -n -f -9 -c arch/arm64/boot/Image > arch/arm64/boot/Image.gz
```

### conn49: гасит USB полностью (FACT)

Повторная прошивка `boot_49_conn49fix.img` (md5 сошёлся при записи) — и
устройство **исчезает с шины USB целиком**: 4 минуты подряд ноль и по adb-VID
`0bb4:0c01`, и по преloader-VID `0e8d:`. Это **отличается** от первого раза,
когда оно оказалось в TWRP, и от поведения сломанного DTB-образа, где преloader
всплывал каждые ~11 с (петля перезагрузок).

**HYPOTHESIS (главная).** Не «ядро не грузится», а **гаснет USB-гаджет**, тогда
как система может работать нормально. Основание: p38 закрывал ровно такой
дефект — `libcomposite/configfs.c` на `device_initcall` забирал класс
`android_usb`, из-за чего `class_create` в `android.c` возвращал `-EEXIST` и
узел не создавался. Порт connectivity добавляет свои `module_init` (`WMT_init`,
`WIFI_init`, `BT_init`, `GPS_init`, `initWlan`, `wmt_detect_driver_init`) и
может воспроизвести тот же класс конфликта или сдвинуть порядок инициализации.
*Проверка:* посмотреть на экран телефона — если там Android, гипотеза верна и
чинить надо USB, а не загрузку.

**HYPOTHESIS (запасная).** Ранний вис до инициализации USB, при этом сторожевой
таймер тоже не срабатывает. *Проверка:* маркеры в зеркале expdb — снят базовый
слепок при живом p74 (md5 `f00299d81a5fe378694310f53fbf7f6a`, первые 4 КБ p10);
если после попытки conn49 слепок изменился, ядро дошло до записи маркеров.

## 2026-08-25 — объединённый образ: всё рабочее в одном, разгон 1352 подтверждён

`boot_49_all.img`, md5 `3532a5a0441eeb7d595f13056777cc7f`, ветка `aud49-fix`.
Собран как p76 + sensors49 + cam49 + av49(без mm-правки) + правка звука + 1352.

### Проверено на железе за одну загрузку (FACT)

Загрузка за **78 с**, дальше всё одновременно:

| подсистема | признак |
|---|---|
| звук | `0 [mtsndcard]: mt-snd-card` |
| датчики | `/dev/gsensor`, `/dev/hwmsensor`, `/dev/msensor`, 7 устройств ввода |
| камера | `/dev/camera-isp`, `/dev/kd_camera_hw` |
| видео | `/dev/Vcodec` |
| разгон | `scaling_available_frequencies` до **1352000** |
| система | `/system` и `/data` смонтированы, zygote работает |

### Пересечение av49 и cam49 — то самое, которое ожидали

Сборка объединённого образа упала на `undefined reference:
register_mmclk_switch_vdec_ctrl_cb`. Причина была документирована заранее в
`drivers/misc/mediatek/smi/Makefile`: при работе над камерой полный
`mmdvfs_mgr.o` заменили на упрощённый `mmdvfs_mgr_default.o`, потому что полный
вызывает `DISP_GetScreenWidth/Height`, требующие `CONFIG_MTK_FB=y`, а дисплей
тогда был выключен. Комментарий прямо говорил вернуть полный, когда дисплей
заработает. Дисплей работает, но видеокодек ссылается на символ, который есть
**только** в полном менеджере. Выбор сделан зависимым от `CONFIG_MTK_FB`, путь
для сборки без дисплея сохранён.

### «Процессор задушен» — это штатный зажим по погасшему экрану (FACT)

Долгая загадка закрыта. Под нагрузкой частота стояла на **1027 МГц** и не
поднималась ни при четырёх ядрах, ни при одном, ни в приоритетной группе
планировщика, ни даже на governor'е **`performance`** — то есть резал драйвер,
а не политика. При этом ограничители чисты:
`/proc/cpufreq/cpufreq_limited_by_pbm` и `..._by_thermal` оба сообщают
`limited_max_freq = 1352000`, бюджет мощности 15,5 Вт, батарея полная, 30 °C.

Разгадка в логе: `@_mt_cpufreq_lcm_status_switch: LCM is off`. Драйвер
понижает потолок, когда экран выключен, а телефон всё это время лежал на стенде
погашенным.

**Контрольный опыт:** `input keyevent KEYCODE_WAKEUP`, затем нагрузка —
частота сразу **1352000**, после гашения экрана снова 1027.

**Вывод:** зажим правильный и штатный, а разгон работает. Важно для методики:
**любые замеры частоты на этом аппарате делать только с включённым экраном**,
иначе они систематически врут.

### Побочно: энергомодель планировщика пуста

`Invalid sched_group_energy for Cluster0/Cluster1` — `sge_array` не заполнен,
то есть у EAS нет энергомодели кластеров (в стоковом DTB нет узлов
`energy-costs`, которых ждёт код 4.9). На выбор частоты это не влияет (доказано
опытом с `performance`), но энергоэффективное размещение задач не работает.
Тот же класс расхождения со стоком; отдельный необязательный фронт.

## 2026-08-25 (вечер) — бисект связи и разбор камеры

### Связь: бисект по конфигурации, два шага сделано

Каждый образ собран на объединённой базе, слит один и тот же код conn49,
менялась только конфигурация. Критерий: доходит ли до системы или сваливается
в TWRP.

| шаг | что включено | результат |
|---|---|---|
| 1 | всё, кроме wlan gen2 | **не грузится** |
| 2 | только ядро WMT/STP/BTIF/CONSYS + BT (без wlan, gps, fm) | **не грузится** |
| 3 | код слит, все конфигурации связи выключены | собирается |

**Вывод после двух шагов (FACT):** виноват **не** wlan gen2 и **не** GPS/FM —
дело в ядре WMT/STP/BTIF/CONSYS. Шаг 3 разделит код и конфигурацию: если с
выключенными конфигурациями образ загрузится, значит вредит конкретная группа
опций, и её можно делить дальше; если нет — вредят сами изменения кода
(Kconfig/Makefile или общие файлы).

### Камера: причина найдена, ядро ни при чём

**FACT.** `cameraserver` не работает вовсе — падает по кругу.
Стек: `CameraService::onFirstRef` → `CameraModule::init` →
`getNumberOfCameras` → `CamDeviceManagerImp::enumDeviceLocked` →
`HalSensorList::searchSensors` → `ImgSensorDrv::impSearchSensor` →
**падение в `ImgSensorDrv::getCurrentSensorType`**, SIGSEGV,
`fault addr 0x24`, `r0 = 0` — разыменование нуля. В `r1` при этом лежит
`0xc0186905`, то есть ioctl `_IOWR('i', 5, 24 байта)` — 32-битная
`COMPAT_ACDK_SENSOR_GETINFO_STRUCT`.

**FACT — ядро отрабатывает правильно.** В dmesg видно, что поиск идёт и
основную камеру находит: `s5k5e8yx[get_imgsensor_id] i2c write id: 0x20,
sensor id: 0x4088` — это отвечает **S5K4H8** (основная). Фронтальную не находит
ни на одном адресе (0x78, 0x20, 0x5a), после чего драйвер штатно ставит
`*sensor_id = 0xFFFFFFFF` и возвращает `ERROR_SENSOR_CONNECT_FAIL` — ровно то,
что предписывает контракт MTK. HAL получает `Err-ctrlCode (I/O error)`, **всё
равно** идёт за информацией и падает на пустом указателе. Это дефект блоба.

**ОТВЕРГНУТО — ошибка в переводе compat-структуры.** В
`compat_get_acdk_sensor_getinfo_struct` строки для `pConfig[0..1]` читают
`pInfo[0..1]` вместо `pConfig[0..1]` — настоящая опечатка. Но **точно такая же
опечатка есть в стоковом 3.18**, где основная камера работала. Значит HAL
рассчитывает именно на это поведение; трогать нельзя.

**HYPOTHESIS (следующий опыт).** На 3.18 слот SUB вообще не опрашивался
(известная запись: «фронталка — слот SUB не опрашивают, имя драйвера»), поэтому
HAL никогда не доходил до этого пути и основная камера работала. На 4.9 мы
зарегистрировали `s5k5e8yx_mipi_raw`, HAL начал опрашивать SUB, получил жёсткий
отказ и падает — утаскивая с собой и рабочую основную камеру.

*Проверка:* убрать `s5k5e8yx_mipi_raw` из
...
## 2026-08-25 (поздно) — связь: настоящий диагноз — вторичные CPU не стартуют; кандидат — адрес `secondary_entry`

### Часы-маркеры перевернули картину (FACT)

В DRAM-окнах уже стояли часы (p29/p37): слот 95 — WDT-кикер (`loops | now_s<<32`),
96–99 — `sched_clock()` последнего тика по CPU0..3, 91/92/93 — вход
`do_cpu_down`/`do_cpu_up`/выход, 88/89/90 — фазы `mt_psci_cpu_boot`.
Три независимых падения (mk2/mk3/mk4) дали одно и то же:

| что | значение |
|---|---|
| WDT-кикер | 4 цикла, замер на ~4 с |
| последний тик CPU0 / CPU1 | 3,2 с / 3,0 с |
| **последний тик CPU2 / CPU3** | **1,10 с / 1,24 с** |
| `do_cpu_down` (hps → CPU2) | 2,12 с |
| `do_cpu_up` → возврат | 3,29 с → 3,31 с (24 мс) |
| `cpu_boot`: psci `cpu_on` вернулся, MTCMOS ON вернулся | 3,313 с, маркеры 25/26/27 есть |

CPU2 и CPU3 **перестали тикать на ~1,1–1,2 с — до всех действий `hps`**;
`hps` на 2,13 с выключил уже мёртвое ядро (`may not have shut down cleanly: 1`),
`cpu_up` на 3,29 с получил «успех» от ATF за 24 мс, но ядро не пришло.

### Та же болезнь в живой conn49-загрузке (FACT)

В `m5c_conn49_kmsg.txt` (COMBO=y, дошла до adb) на **1,09 / 2,15 / 3,20 с**:
`CPU1/2/3: failed to come online`, `failed in unknown state : 0x0`
(= не выполнено ни одной инструкции ядра). На рабочем `all`:
`CPU2/3: Booted secondary processor` на **0,078 с**. Это **до любого
initcall connectivity** — поэтому весь разбор WMT/STP/PSM/BTM был мимо цели:
код connectivity ни при чём, он только **сдвигает раскладку ядра**.

### Что отличает падающие образы от рабочих — физический адрес `secondary_entry` (FACT)

`enable-method = "mt-boot"` → `mt_psci_cpu_boot` → `psci_ops.cpu_on(cpu,
__pa_symbol(secondary_entry))` → стоковый ATF. У стока 3.18
`secondary_entry = _text + 0x1c0` (в самом начале ядра). В 4.9 `.idmap.text`
линкуется **после** всего `.text`, и `secondary_entry` уезжает на 10–12 МБ.
Сигнатурный поиск по всем образам дня:

| `secondary_entry` (phys) | образ | грузился |
|---|---|---|
| 0x40a151a0 | p74 | да |
| 0x40aa81a0 | audfix | да |
| 0x40ac01a0 | conn49fix | **нет** (контрпример, без маркеров) |
| 0x40b411a0 | bis3 (код слит, связь выкл.) | да |
| 0x40b441a0 | all | да |
| 0x40b4c1a0 | bis4 (BTIF) | да |
| 0x40b741a0 | bis2, bis5 | нет |
| 0x40b921a0 | bis1 | нет |
| 0x40be81a0 … 0x40bec1a0 | noidc, kmark2/3/4, connfix | нет |

Порог между `0x40b4c1a0` и `0x40b741a0`. Один контрпример (conn49fix на
«низком» адресе) — тот образ падал до появления часов-маркеров и должен быть
переснят; он же единственный на «сыром» conn49 без правок этой сессии.

Отвергнуто по дороге: лимит размера образа (стоковое ядро 17,46 МБ больше
нашего `all`), `spm_mtcmos_ctrl_cpu2` (идентичен стоку), `mt-smp.c`
(в 4.9 не линкуется — `mt_smp_prepare_cpus` отсутствует в vmlinux),
дубль `gConEmiPhyBase` из `connectivity_build_in_adapter.c` (адаптер
сознательно не линкуется, см. `connectivity/Makefile:6`), SDIO-скан msdc1
(`sdio_register_driver` — только `driver_register`; без карты `mmc_rescan`
не включает питание), резервирование `consys-reserve-memory` (одинаково
при COMBO=y/n).

### Два решающих образа, готовы к прошивке (ожидания записаны заранее)

| образ | md5 | связь | `secondary_entry` | если гипотеза верна |
|---|---|---|---|---|
| `boot_49_padtest.img` | `875668a71bb5702adb8e461140510188` | **нет** | 0x40c441a0 (1 МБ паддинга `.text` перед idmap) | **не грузится** → COMBO оправдан |
| `boot_49_idmaplow.img` | `b214e599c31ac131f035ad0be4678139` | **полная** (BT+WiFi+GPS+FM, маркеры) | 0x400841a0 (`IDMAP_TEXT` перед `TEXT_TEXT`, коммит на `conn-bisect1`) | **грузится** → готовое исправление |

Оба через гейт (`mt6735m-mmc` ×2, `mediatek,msdc` ×0). Если оба исхода
совпадут с ожиданием — причина доказана двусторонне: **стоковый ATF не
запускает вторичное ядро с entry выше ~0x40b5xxxx**, и лечится
переносом `.idmap.text` в начало `.text` (одна строка `vmlinux.lds.S`).
Если `padtest` загрузится — адрес ни при чём, возвращаемся к COMBO с
часами-маркерами на conn49fix.

Дополнительно заготовлен `dump_atf.sh` (снять bl31 из `0x43000000`, 192 КБ,
в TWRP) — чтобы найти проверку диапазона `cpu_on` в самом ATF.

### Процедурное

Телефон после прошивки `kmark4` ушёл с USB полностью (ни adb, ни preloader)
и в TWRP сам не попадает — нужны руки. Обнуление окон маркеров через
`dd of=/dev/mem` в TWRP не подтверждено (снимок остался от 452-секундной
сессии `all`); отличать свежий прогон по слотам 97/99 (`0xbf2xxxxx` /
`0x41xxxxxx`), которые старые ядра не ставят.

## 2026-08-26 — пара решающих опытов: адрес `secondary_entry` ОТВЕРГНУТ; связь остаётся

### Результат (FACT)

| образ | связь | `secondary_entry` | ожидание | **факт** |
|---|---|---|---|---|
| `padtest` (1 МБ паддинг `.text` перед idmap) | нет | 0x40c441a0 | не грузится | **грузится, 78 с, online=0-3** |
| `idmaplow` (`IDMAP_TEXT` перед `TEXT_TEXT`) | полная | 0x400841a0 | грузится | **не грузится, TWRP на 72 с** |

Оба исхода противоположны ожиданию → **физический адрес `secondary_entry`
не при чём**, и раскладка `.data` тоже (у `padtest` `_end`≈0x41100008 выше,
чем у `all`, и всё работает). Монотонность таблицы адресов из прошлой
записи была совпадением с ростом ядра от связи. Коммит `a03961898`
(перенос idmap) — оставлен как безвредный эксперимент; можно снять.

### Что осталось после этого (FACT, статика)

- `autoconf.h` шаг4 (грузится) → шаг5 (нет) отличается ровно четырьмя
  макросами: `CONFIG_MTK_COMBO`, `_CHIP`, `_CHIP_CONSYS_6735`, `_PLAT_PATH`.
- **Ни один `.c`/`.S` вне `drivers/misc/mediatek/connectivity/` их не
  использует** (проверено grep по `drivers/ arch/ kernel/ sound/ net/`).
  `mtk_wcn_cmb_stub.h` из мержа — только typedef-алиасы.
- Значит код вне connectivity при COMBO=y байт-в-байт тот же, а вторичные CPU
  не стартуют (`failed in unknown state 0x0` на 1,09 с). Единственное, что
  из connectivity исполняется **до `smp_init`**, — `reserve_memory_consys_fn`
  (`RESERVEDMEM_OF_DECLARE`), и он лишь сохраняет `rmem->base`.

### Самострел инструментовки (записать, чтобы не повторить)

`forge_kmark_ptr(97/99)` в `reserve_memory_consys_fn` — вызывается из
`arm64_memblock_init` **до `paging_init`**; `phys_to_virt` там невалиден →
образ `kmark4` умирал раньше, чем `head.S` ставил свои слоты, а каждый
последующий TWRP-снимок показывал окно **предыдущей** сессии (WDT≈450 с).
Убрано (`conn-bisect1`). Общее правило: маркеры-значения — только после
`paging_init`; свежесть снимка проверять по слоту 95 (WDT-loops должны быть
единицами, не сотнями).

Также: `mknod /dev/mem` + `dd` в TWRP роняет recovery (устройство
пропадает с USB) — ATF-дамп через `/dev/mem` из TWRP не делать.

### Следующий инструмент (не маркеры)

Вторичные CPU надо ловить там, где они умирают: **в `secondary_start_kernel`
/ `head.S` `secondary_startup`**, а не в `module_init`. Варианты:
1. `__early_cpu_boot_status` — читается `__cpu_up` и уже печатается
   (`unknown state : 0x0` = CPU_MMU_OFF не достигнут → ядро не выполнило
   **ни одной** инструкции ядра). Это значит: либо ATF не выпускает ядро из
   WFI (PSCI `cpu_on` вернул успех, но entry не прыгнул), либо ядро прыгнуло
   и сразу упало до первой записи статуса.
2. Прямая проверка ATF: **сравнить `psci_ops.cpu_on` аргументы** между
   `all` и `kmark`: `cpu_logical_map(cpu)` (MPIDR) — одинаково; entry —
   отвергнуто. Остаётся **состояние SPM/MTCMOS CPU2/3 до `cpu_on`**:
   `spm_mtcmos_cpu_init()` в `cpu_prepare` (маркеры 23/24 стоят) и
   `SPM_CA7_CPU2_PWR_CON`/`SPM_PWR_STATUS` — снять регистры на живом `all`
   (`/dev/mem` **в Android**, не в TWRP) и сравнить со значениями,
   которые пишет `spm_mtcmos_ctrl_cpu2(ON)`.
3. Если и это совпадёт — включить `CONFIG_DEBUG_LL`/early UART на
...
## 2026-08-25 (ночь, 2) — `cam5`: конфликт VGP1 снят, тач жив на 1.2 В; дисплейная модель CMDQ-гейта ОТВЕРГНУТА количественно

`cam5` = `all4` + `50d492f4a` (легаси-номер MCLK-ioctl, VGP1 по имени) +
`36d99f450` (тач не пиннит VGP1 на 2.8 В; маска реально включённых рельс).
md5 `e4ee3b26ff0b66bea266bb0041378cc0`, readback совпал, Android грузится.

### Тач переживает 1.2 В на VGP1 — гипотеза ПОДТВЕРЖДЕНА (FACT)

```
vgp1 enabled users=2 uv=1200000        (потребители: тач + камера)
[mtk-tpd] gt9xx touch key -> code 158 down / code 158 up      <- реальные касания
input: mtk-tpd (event5), mtk-tpd-kpd (event6) на месте
```
То есть панель тача физически не питается от VGP1 (как и в стоке, где
consumer'а `vtouch` нет вовсе, а камера роняет VGP1 до 1.2 В на каждом
запуске фронталки). Замена точечного запроса 2.8 В на диапазон
1.2–2.8 В в `gt9xx_driver.c` регресса не дала.

### Питание фронталки подаётся полностью (FACT)

```
[84.239] [kd_camera_hw] power ON pinSetIdx=0 sensor=s5k5e8stmipiraw
[84.240] [_hwPowerOn] type=0 2800000uV ok      (VCAMA)
[84.241] [_hwPowerOn] type=4 1200000uV ok      (SUB_VCAMD = VGP1)
[84.243] [_hwPowerOn] type=2 1800000uV ok      (VCAMIO)
[88.290] s5k5e8yx[get_imgsensor_id] i2c write id: 0x78, sensor id: 0x0
```
Ни `Restricting voltage`, ни `Fail to OFF`, ни `unbalanced disables`.
**Новая зацепка:** в строке питания `pinSetIdx=0` — нулевой набор пинов,
т.е. набор ГЛАВНОЙ камеры. Ветка s5k5e8 поднимает `CAMRST` набора 0, и
фронтальный сенсор остаётся в сбросе при полностью поданном питании —
что в точности совпадает с наблюдаемым «шина отвечает, id = 0x0».
Отдано в работу; версия «сенсор деградировал» отложена до исправления
адресации пинов.

### Дисплей: модель «залипший `DISP_RDMA0_EOF`» ОТВЕРГНУТА (FACT)

`/proc/mtk_cmdq_debug/record` под активным скроллом даёт время ожидания
задач, размазанное 18 мкс … 16 мс (19/3/0.2/2/16/14/11/1 мс …), то есть
`WFE` реально ждёт событие, а не проскакивает. Вместе с A/B
(`gate_line=1280` одинаково при `forgegate:0` и `1`) и живым heartbeat
trigger loop это закрывает версию (B').

Отвергнуто по дисплею на сегодня: underflow (C), тайминги DSI (D),
stride/pitch (`forge-pitch` = 720, `L0_PITCH`=0xb40=720×4), залипший
`RDMA0_EOF` (B'). Артефакты сборки, не улики: `MUTEX=0` и почти
неподвижный `OVL0` в счётчиках прерываний — `_MTK_USER_` держит
`gEnableIRQ=0`, из-за чего OVL0 `INTEN=0x1e0` (только аномальный EOF), а
INTEN мьютекса выключен (`ddp_path.c:866`).
...
### `cam8`: HAL просит drvIdx=0 на ГЛАВНОМ сокете — индекс 0 у нас фронталка (FACT)

Инструментовка интерфейса драйверов сенсоров (`SET_DRIVER`,
`GET_SOCKET_POS`, `CHECK_IS_ALIVE` через `pr_info`) дала однозначный ответ:
```
[kd_sensorlist] SET_DRIVER raw[0]=0x00010000 (socket=1 drvIdx=0) raw[1]=0x00000000
[kd_sensorlist] SET_DRIVER slot0: socket=1 drvIdx=0
[kd_sensorlist] CHECK_IS_ALIVE: slot0 socket=1 en=1 name=s5k5e8stmipiraw | slot1 socket=0 en=0
```
и так в каждом цикле перезапуска mediaserver.

- `socket=1` = `DUAL_CAMERA_MAIN_SENSOR` (→ `pinSetIdx=0`), т.е. запрос
  идёт на ГЛАВНЫЙ сокет.
- `drvIdx=0` — индекс в нашем `kdSensorList`, а нулевой элемент у нас
  `S5K5E8_ST` (список начинается с четырёх вариантов фронталки). Отсюда
  `name=s5k5e8stmipiraw` на главном сокете: драйвер фронтальной камеры
  натравлен на заднюю.
- Других индексов HAL **не пробует ни разу**, второй слот всегда пуст.
  То есть перебора списка нет: он спрашивает конкретный индекс, не находит
  сенсор и падает в `getCurrentSensorType` с невыделенными буферами.

INFERENCE: предпосылка комментария в `kd_sensorlist.h` (коммит `cdd3becb2`)
— «блоб адресует драйверы по индексу, стоковый порядок: четыре S5K5E8,
затем четыре S5K4H8» — противоречит поведению живого блоба, который просит
индекс 0 для ГЛАВНОЙ камеры. Возможные объяснения (проверяются): порядок
верен для стокового Flyme-HAL, но наш ROM несёт другой набор блобов; либо
индекс приходит из userspace-конфигурации ROM, а не из ядра. До выяснения
механизма порядок списка вслепую не менять.

### Порядок списка сенсоров ВЕРЕН — подозрение снято (FACT)

Строки имён драйверов внутри ROM-блоба `/system/lib/libcameracustom.so`
идут в том же порядке, что и наш `kdSensorList`: S5K5E8_ST, _QH,
_HOLITECH, _SUNWIN, затем S5K4H8_OFILM, _ST, _HOLITECH, _SUNWIN. Значит
индекс 0 = S5K5E8_ST — ровно то, чего блоб и ждёт, и порядок менять не
надо.

Тогда поведение HAL логично: он перебирает список с нуля и первым
проверяет фронтальный драйвер на ГЛАВНОМ сокете (обычный поиск «кто
отвечает на этом сокете»). Ядро честно отвечает «не отвечает»
(`SENSOR_FEATURE_CHECK_SENSOR_ID` → 0xFFFFFFFF →
`ERROR_SENSOR_CONNECT_FAIL`), после чего HAL обязан перейти к следующему
индексу — но вместо этого падает. Открытый вопрос сузился до одного:
возвращаем ли мы на неудачном кандидате не то, что возвращал сток (тогда
чинится в ядре), или блоб всегда падал на первой неудаче, а в стоке
первый кандидат просто отвечал (тогда починка фронталки чинит и главную
камеру заодно).

### Почему HAL не доходит до sub-фазы: он падает на первой же пробе (FACT, дизасм)

Полный разбор `ImgSensorDrv::impSearchSensor` (0x135f8, ARM32) —
двухфазный цикл: инициализация `sl=2, r7=1`; внутренний цикл `r4=0..15` по
записям списка (шаг 56 байт, поле +44 = `SensorInit`, NULL-слоты
пропускаются); смена фаз на 0x13942 (`cmp sl,#1; r7 = r7<<1` → фаза A с
сокетом 1 = MAIN, фаза B с сокетом 2 = SUB). Аргумент SET_DRIVER строится
как `(socket<<16)|drvIdx` — ровно то, что читает наш `kdSetDriver`.

Тело пробы (0x1372e … 0x137a0):
```
1374e: ioctl SET_DRIVER          ; при ret<0 только лог, ПРОДОЛЖАЕТ (0x13752)
1376a: ioctl CHECK_IS_ALIVE      ; при ret<0 только лог + strerror, ПРОДОЛЖАЕТ (0x1376e)
1378a: ldr r3,[r9] ; ldr r2,[r3,#0x4c] ; blx r2      <- getCurrentSensorType, БЕЗУСЛОВНО
1379c: getSocketPosition(...)
```
То есть неудачный кандидат не прерывает перебор — но `getCurrentSensorType`
вызывается **безусловно** сразу после проверки, и именно он падает
(`ldr r6,[r5,#0x348]` → `ldr r1,[r6,#0x24]`, SIGSEGV). Поэтому цикл
умирает на первой итерации: в логе всегда только `drvIdx=0`, фаза SUB не
наступает никогда, и фронталка не проверяется в принципе.

Наша сторона на этом пути выглядит корректной: `adopt_CAMERA_HW_CheckIsAlive`
возвращает `-EIO` при ненайденном сенсоре, `adopt_CAMERA_HW_GetInfo`
возвращает `-EFAULT`, если любой из указателей `pInfo[0..1]`/`pConfig[0..1]`
равен NULL. Но HAL уходит по ветке УСПЕХА (`cbz r0` на 0x1170c), значит
какой-то запрос перед падением получил от нас 0. Чтобы не гадать, в ядро
добавлен сквозной лог: имя/номер/размер каждой команды и её результат на
нативном пути, плюс команда и любое проваливание в `default` на compat-пути
(блоб 32-битный на 64-битном ядре, всё идёт через `compat_ioctl`).

Также проверено и отвергнуто: `kdGetSocketPostion` идентичен стоковому
(MAIN→1, MAIN_2→2, SUB→−1), декодер `socket = (raw & 0xFFFF0000) >> 16` в
`kdSetDriver` совпадает со стоком байт-в-байт, транспорт sub в ядре готов
(второй platform-драйвер на `mediatek,camera_sub` = `camera_sub@3c`,
`i2c_add_driver` в probe2, `iReadRegI2C` умеет BUS2).

Ещё из стокового дизасма: в стоке ветка питания S5K5E8 закрыта проверкой
`pinSetIdx == 1` (0x5bc044: `cmp w19,#1; b.ne generic`), CMPDN для неё не
дёргается вовсе — обе особенности воспроизведены.

### КОРЕНЬ отказа камеры: ядро отвечает «успех» с нулями на GETINFO после ненайденного сенсора (FACT)

Сквозной лог ioctl (образ `cam9`, md5 `bdff52993befe886866cd31794889f05`)
дал точную последовательность перед каждым крашем mediaserver:
```
compat 0xc008693c (nr=60 size=8)  -> ioctl OUT ret=0     SET_MCLK_PLL (legacy-номер работает)
compat 0xc008693c                 -> ret=0
compat 0xc0086923 (nr=35 size=8)  -> SET_DRIVER raw[0]=0x00010000 (socket=1 drvIdx=0), ret=0
compat 0x0000691e (nr=30 size=0)  -> CHECK_IS_ALIVE slot0 socket=1 name=s5k5e8stmipiraw
                                     ioctl OUT ret=-5        <- сенсор НЕ найден
compat 0xc0186905 (nr=5 size=24)  -> ioctl IN cmd=0xc0286905 (nr=5 size=40)
                                     ioctl OUT ret=0         <- УСПЕХ на GETINFO
                                     (далее SIGSEGV в getCurrentSensorType)
```
Compat-конверсия GETINFO корректна (24 → 40 байт). Дефект в другом:
`kd_MultiSensorGetInfo` (kd_sensorlist.c:819) обнуляет локальные структуры,
и когда ни один активный драйвер их не заполнил, копирует наружу НУЛИ,
возвращая `ERROR_NONE`; `adopt_CAMERA_HW_GetInfo` при этом вообще не
проверяет возврат `SensorGetInfo` и отдаёт 0. HAL получает «успех» с
нулями, берёт из структуры поле как указатель (r6 = 0) и выполняет
`ldr r1,[r6,#0x24]` → SIGSEGV по адресу 0x24 — ровно наблюдаемый краш.

Следствие: перебор сенсоров умирает на первом же кандидате, до S5K4H8
(индекс 5) и до sub-фазы дело не доходит никогда. Лечение — вернуть
ошибку на GETINFO, когда сенсор не найден/не инициализирован, как это
делает сток; тогда HAL уйдёт по своей ветке ошибки и продолжит перебор.
...
## 2026-08-25 (ночь, 5) — КАМЕРА: настоящий корень — расхождение ABI структуры sensor-info

Предыдущее объяснение («ядро вернуло успех с нулями») было верным по
симптому, но не по механизму. Настоящая причина, найденная по дизасму
блоба и разметке структур:

- В этом дереве `ACDK_SENSOR_INFO_STRUCT` — поздняя ревизия BSP: **212
  байт**, с 18 дополнительными полями `MUINT16` grab-start, вставленными
  сразу после `SensorGrabStartY` (смещение 96), и ещё несколькими дальше.
- Блобы 2016 года, которые несёт этот ROM, собраны под исходную раскладку
  mt6735: `GETINFO` ждёт **144 байта**, `GETINFO2` — **192**.
- HAL держит оба выходных буфера `GETINFO` ВНУТРИ объекта `ImgSensorDrv`,
  по 144 байта каждый, и сразу за ними лежат массивы указателей
  `pInfo`/`pConfig` (смещения объекта 0x228/0x2b8, указатели по 0x348;
  конструктор 0x11ba6 в `libcam.halsensor.so`).

Следствие: `copy_to_user` на `sizeof(наша структура)` = 212 байт
(а) сдвигал каждое поле после смещения 96 и (б) **переполнял второй буфер
на 68 байт, обнуляя эти самые указатели**. После чего
`ImgSensorDrv::getCurrentSensorType` разыменовывал NULL+0x24 —
`mediaserver` уходил в цикл падений ещё до конца поиска сенсоров. Отсюда
и полная неработоспособность камеры, и то, что фронтальный сокет не
опрашивался ни разу.

Лечение: не копировать структуру ядра наружу, а маршалить её в точные
legacy-раскладки (списки полей взяты из 3.18-дерева, под которое собраны
...
## 2026-08-25 — ИТОГОВАЯ ПРОВЕРКА образа `md13` (эталон на конец сессии)

`md13`, md5 `bc138d6e5a62e9369c667f5e88e05fb8`, содержит ВСЕ правки сессии.
Прошит в p7, readback совпал, холодная перезагрузка, одна сессия проверки:

```
uname -r          4.9.188-m5c+          cpus online 0-2 (hotplug штатный)
Wi-Fi             wlan0 d8:6c:02:ab:6f:3e, dumpsys wifi: 17 точек в скане
Bluetooth         enabled: true   state: ON
Камера            Number of camera devices: 1   Facing: BACK
Аудио             /dev/snd: adsp audio controlC0 ...
Тач               mtk-tpd + mtk-tpd-kpd в /proc/bus/input/devices
```

### Что было починено за сессию (все причины — расхождения порта 3.18→4.9 с ожиданиями стоковых блобов)

1. **Связь (Wi-Fi/BT/GPS-узел).** `f_op->read` == NULL на ext4 в 4.9 →
   `WMT_SOC.cfg` не читался (`kernel_read`); error-path `wmt_plat_deinit`
   падал на незарегистрированном wakeup source (флаг состояния);
   BTIF DMA брал регистры из узла не той формы (поиск отдельных узлов
   `btif_tx`/`btif_rx` + отказ работать с нулевой базой).
2. **Камера.** ABI `ACDK_SENSOR_INFO_STRUCT`: 212 байт в дереве против
   144/192, которые ждут блобы — `copy_to_user` переполнял буферы HAL и
   затирал его массивы указателей (маршалинг в legacy-раскладку);
   номер `KDIMGSENSORIOC_X_SET_MCLK_PLL` разошёлся со стоковым из-за поля
   TG; питание фронталки по стоковой схеме (VGP1 1.2 В вместо VCAMD) и
   гейт по `pinSetIdx == 1`; тач перестал пиннить общую линию VGP1 на 2.8 В.
3. **Модем.** Правки уходили в несобираемый каталог `eccci/` (при
   `MACH_MT6735M` собирается `eccci1/`); добавлены семь узлов
   `/dev/ccci_*`, которые открывает стоковый демон; опубликован
   legacy-триггер `/sys/class/BOOT/BOOT/boot/md`. Модем грузит образ,
   поднимает VLTE, стартует CLDMA и отвечает хендшейком HS1.
4. **Ядро/отладка.** MD1-reserve перед сканом reserved-memory; маркеры
   вторичных CPU + `/proc/forge_marks`; сквозной лог ioctl камеры;
   диагностика CMDQ-гейта и честная метрика строки развёртки.

### Открытые линии (в работе)

- **Модем:** exception через ~65 с после старта (`MD exception logical
  0→3`), EE-текст в dmesg не появляется — искать дампер. Плюс замечено
  `init: write_file: Unable to open '/sys/class/misc/md32/md32_boot'`.
- **Фронтальная камера:** sub-фаза исполняется, сенсор молчит; кандидаты —
  MCLK второго тактового выхода и шина/адрес `camera_sub@3c` (client2).
- **Разрывы изображения:** ядро исключено доказательно; причина в
  юзерспейсе (fence/контент), нужна пересборка `hwcomposer`-.so.
- **GPS:** ядро исправно, `mtk_agpsd` не линкуется (ICU-55) — ROM-shim.
- **ROM:** `init.modem.rc` с chown для новых узлов требует пересборки
  ramdisk (сейчас права выставляются вручную для теста).

### Точка остановки: устройство пропало с USB

После проверки `md13` телефон перестал определяться (`lsusb` не видит
`0e8d`, adb пуст) — вероятнее всего разряд или физическое отключение,
нужен человек у аппарата. **В p7 лежит проверенный рабочий `md13`**
(md5 `bc138d6e5a62e9369c667f5e88e05fb8`): Wi-Fi, BT, камера, звук, тач
подтверждены на нём одной сессией. Ничего не сломано.

Готов, но НЕ прошит: `cam14`
(md5 `30e48b0f8286dfce11dbcdb9f58b0f5c`) — то же самое плюс инструментовка
i2c для охоты на фронталку (печатает адаптер/адрес/шину при каждом отказе
пробы и обе привязки клиентов при probe). Лежит в скретчпаде сессии,
прошивать первым при возвращении устройства.

Что снять сразу после прошивки `cam14`:
```
dmesg | grep -E "client1 \(camera_main\)|client2 \(camera_sub\)|iReadRegI2C|SET_DRIVER slot"
```
Это ответит, привязан ли клиент `camera_sub@3c`, на какой адаптер и адрес
реально уходят чтения sub-сенсора и какая шина выбрана — три версии,
между которыми сейчас выбирают по фронтальной камере.

### Закрытые вопросы по камере из финальных отчётов (FACT, стоковый дизасм)

- **MCLK для фронталки — правка НЕ нужна.** Стоковая ветка питания S5K5E8
  включает `ISP_MCLK1_EN` — тот же гейт, что и у главной камеры
  (хвост power-on 0x5bced0: `mov w0,#1; bl ISP_MCLK1_EN`; power-off
  0x5bd4dc / 0x5bd624 — `ISP_MCLK1_EN(0)`). `ISP_MCLK2_EN` в стоковом
  `kdCISModulePowerOn` не вызывается нигде, а стоковый `kdSetSensorMclk`
  работает только с `MT_MUX_CAMTG` (`enable_mux(4)` + `clkmux_sel(4)`
  @0x5b9a10). То есть на этой плате обе камеры сидят на одном
  CAMTG/MCLK1, и наш код этому соответствует. Версию «фронталке нужен
  второй тактовый выход» закрыть.
- **Таблица адресов sub-сенсора одна на все четыре варианта:**
  `{0x78, 0x20, 0x5a, 0xff}` (`s5k5e8yxmipiraw_Sensor.c:183`), 0x78 —
  первый, цикл пропустить его не может. Наблюдение «в логе только 0x20 и
  0x5a» — почти наверняка обрезка выборки/потеря строк printk, проверяется
  на полном dmesg (`grep -c "i2c write id: 0x78"`).
- **Возвращать ошибку на GETINFO при ненайденном сенсоре НЕ надо** — сток в
  этой ситуации возвращает успех (драйвер кандидата привязан, наружу уходят
  его статические капы, а не нули), и блоб такой успех штатно переваривает.
  Чинились данные, а не ветка возврата.
- Попутно в том же коммите исправлена копипаста MTK в
  `compat_get_acdk_sensor_getinfo_struct`: он читал `pInfo[0/1]` вместо
  `pConfig[0/1]`, из-за чего 64-битный обработчик писал config-структуры
  поверх info-буферов HAL, а настоящие config-буферы не заполнялись никогда.
- Хвост на будущее: `KDIMGSENSORIOC_X_GET_ISP_CLK` возвращает 0 с
  незаполненным int (запись результата закомментирована). Там не указатель,
  блоб его не разыменовывает, поэтому не критично — но неверно.

## 2026-08-27 — фронтальная камера: сенсор ОТВЕЧАЕТ по i2c, но держится в сбросе

### Доступ к устройству: узел USB без прав (записать, чтобы не терять время)

Телефон «пропадал с USB» не по своей вине: на девбоксе `/etc/udev/rules.d`
пуст (узлы создаёт хост), поэтому USB-узел m5c каждый раз появляется как
`root:root`, и adb его не видит, хотя `lsusb` показывает устройство и
`/sys/.../serial` читается. Лечится одной строкой:
`sudo chmod 666 /dev/bus/usb/<bus>/<dev>` — номер узла меняется после
КАЖДОЙ перезагрузки. Хелпер `usbfix.sh` (в скретчпаде) находит узел по
серийнику, открывает его и перезапускает adb-сервер только при
необходимости. Симптом-обманка: VID:PID показывается как `0bb4:0c01`
(дефолтный Android gadget), интерфейс при этом `ff/42/01` — это ADB.

### Восстановление упаковки после чистки скретчпада

`dtb_stock.dtb` и базовый `boot.img` восстанавливаются из уже прошитого
образа: `adb pull /data/local/tmp/b13.img`, `abootimg -x`, затем разрезать
`zImage` по сигнатуре DTB `d00dfeed` — получаются `Image.gz` (7747411 б) и
`dtb_stock.dtb` (69427 б). Пересобранный этим способом `cam14` дал md5
`30e48b0f8286dfce11dbcdb9f58b0f5c`, байт-в-байт совпав с записанным ранее.

### Транспорт sub-камеры исправен (FACT, образ `cam14`)

```
[kd_sensorlist] client1 (camera_main) bound: i2c-0 dt-addr=0x10 timing=200kHz
[kd_sensorlist] client2 (camera_sub)  bound: i2c-0 dt-addr=0x3c timing=100kHz
```
Оба клиента на одном адаптере, sub на 0x3c (= write id 0x78), как в
стоковом DTB. Версия «client2 не забиндился» — REJECTED.

### Главное: сенсор ACK'ает, но регистры читаются нулями (FACT)

```
s5k5e8yx[get_imgsensor_id] searching sub sensor, expecting id 0x5e80
s5k5e8yx[get_imgsensor_id] i2c write id: 0x78, sensor id: 0x0     <- НЕТ "send fail"
s5k5e8yx[get_imgsensor_id] i2c write id: 0x20, sensor id: 0x4088  <- отвечает ГЛАВНАЯ
s5k5e8yx[get_imgsensor_id] i2c write id: 0x5a, sensor id: 0x0
[kd_camera_hw] power ON pinSetIdx=0 sensor=s5k5e8stmipiraw
```
Адрес 0x78 **пробуется** (прежнее «0x78 не участвует» было обрезкой
выборки) и **не даёт ошибки шины** — при том, что наша инструментовка
печатает любой сбой передачи с адаптером и слейвом. Значит на 0x3c есть
ACK: сенсор запитан и адресуем, но его регистры идентификации возвращают
0 — картина сенсора, удерживаемого в сбросе / без такта.

Причина найдена в нашей же логике: проба идёт с `pinSetIdx=0` (MAIN-фаза),
а ветка стокового питания фронталки закрыта гейтом `pinSetIdx == 1`.
В итоге фронталка получает generic-ветку: VCAMD 1.5 В (не её линия — её
ядро висит на VGP1) и сброс cam0 вместо cam1.

**Почему стоковый гейт здесь неприменим:** в стоке вторая фаза поиска
реально приходила с сокетом 2, а у нас она приходит пустой —
`SET_DRIVER slot1: socket=2 drvIdx=255` (0xFF = слот не назначен). То есть
фронталку HAL пробует ТОЛЬКО с MAIN-сокета.

Правка: ветка питания фронталки теперь выбирается по имени драйвера, а
пины всегда берутся её собственные (набор 1, состояния `cam1_*`). Сброс
главной камеры опускается в начале ветки в любом случае, поэтому на общей
шине отвечает только одна из двух.

Побочно замечено: при `bus=2` (после `SET_DRIVER slot1 socket=2`) запросы
главного драйвера уходят через client2 и получают `-22 (EINVAL)` —
`send fail bus2 i2c-0 slave=0x10 reg=0x6f12`. Это не NACK, а отказ
драйвера i2c; на работу главной камеры не влияет (её проба идёт при
`bus=0`), но в очередь на разбор.

### `cam15`: ветка питания фронталки теперь исполняется, но шина отдаёт EINVAL

Гейт `pinSetIdx == 1` снят, ветка выбирается по имени драйвера и всегда
дёргает пины набора 1. Лог подтверждает, что всё отработало как задумано:
```
[kd_camera_hw] s5k5e8 stock power branch (sub pins, called with pinSetIdx=0)
[kd_camera_hw] pinctrl cam0_rst0 ret=0
[kd_camera_hw] pinctrl cam1_rst0 ret=0
[_hwPowerOn] type=0 2800000uV ok   (VCAMA)
[_hwPowerOn] type=4 1200000uV ok   (VGP1)
[_hwPowerOn] type=2 1800000uV ok   (VCAMIO)
[kd_camera_hw] pinctrl cam1_rst1 ret=0
```
Главная камера при этом продолжает работать (`0x4088`, открывается,
`Number of camera devices: 1`) — регресса нет.

Но чтения фронталки теперь возвращают не «ACK с нулями», а ошибку шины:
```
[iReadRegI2C] send fail bus1 i2c-0 slave=0x3c reg=0x0000 ret=-22
[iReadRegI2C] send fail bus1 i2c-0 slave=0x3c reg=0x0001 ret=-22
```
`-22` = `-EINVAL`, то есть отказ драйвера i2c, а не NACK сенсора. Отказы
приходят **только** на чтениях S5K5E8 (регистры `0x0000/0x0001`), тогда как
чтения S5K4H8 (`0x602C/0x6F12`) проходят.

### ОТВЕРГНУТО: «драйверы задают кГц, а mt_i2c ждёт Гц»

Гипотеза выглядела убедительно: все сенсорные драйверы дерева объявляют
`i2c_speed` как 200/400 (кГц) и передают в `kdSetI2CSpeed`, который кладёт
значение в `client->timing`, а `i2c-mtk.c` использует его как частоту в
герцах (`speed_hz = i2c->ext_data.timing`), причём 400 Гц ниже того, что
контроллер способен поделить, и `mtk_i2c_calculate_speed()` возвращает
именно `-EINVAL`.

**Проверка на железе опровергла (образ `cam16`, md5
`df8dfbd47c68245b6b45fc84b0130d7e`):** после конвертации кГц→Гц отказов
стало БОЛЬШЕ (336 против 104), и пропала главная камера —
`Number of camera devices: 0`. То есть 400000 Гц шина принимает не лучше,
а хуже. Правка откачена; в единицах дело не в них.

Что это оставляет: `-EINVAL` приходит из другой проверки `mt_i2c`
(кандидаты — `msgs->addr == 0`, длина, режим ext при активном
`I2C_A_FILTER_MSG`), либо из `mt_i2c_clock_enable`. Разбирать по коду
`drivers/i2c/busses/i2c-mtk.c`, а не подбором.

### Источник `-EINVAL` найден: запрошенная скорость выше, чем настроена шина (FACT)

Стоковый DTB объявляет контроллер, на котором сидят обе камеры, как
```
i2c@11007000 { compatible = "mediatek,mt6735m-i2c"; def_speed = <0x64>;   /* = 100 */
    camera_main@10 { reg = <0x10>; }  camera_main_af@18 { }  camera_sub@3c { reg = <0x3c>; } }
```
`kdSetI2CSpeed()` кладёт `imgsensor_info.i2c_speed` прямо в
`client->timing`, а `mt_i2c` программирует шину из этого значения
(`i2c-mtk.c`: `speed_hz = i2c->ext_data.timing`). Драйвер S5K5E8 просит
**400** при шине, настроенной на **100**, — `mtk_i2c_calculate_speed()`
отвергает такую комбинацию, и каждая его передача возвращает `-EINVAL`.
Главная камера с тем же `.i2c_speed = 400` уцелела лишь потому, что
читает свой id ДО первого вызова `kdSetI2CSpeed()` (он у неё стоит в
`s5k4h8_resolve_variant`, после успешного чтения).

Это же объясняет, почему конвертация кГц→Гц сделала хуже: 400000 тем
более выше 100, и тогда сломались ОБЕ камеры (`Number of camera
devices: 0`). Откат подтверждён на железе — главная вернулась
(`devices: 1`, 104 отказа как прежде).

Правка: `.i2c_speed` фронтального драйвера приведён к `def_speed` шины
(100). Правка точечная: главная камера не затрагивается.

### ОТВЕРГНУТО и это: «400 против def_speed 100» (FACT, образ `cam18`)

`.i2c_speed` фронтального драйвера приведён к 100 — **ничего не
изменилось**: те же 104 отказа, та же разбивка
(16 × bus1 0x3c, 16 × bus1 0x10, 16 × bus1 0x2d, 20 × bus2 0x10,
20 × bus2 0x2d, 16 × bus2 0x3c), главная камера работает.
Правка откачена как не давшая эффекта.

Итого по `-EINVAL` отвергнуты обе версии про скорость: и «кГц против Гц»
(стало хуже, обе камеры отвалились), и «выше def_speed» (без изменений).

### Что известно об этом `-EINVAL` точно (для следующего захода)

- Отказ приходит **только** от драйвера S5K5E8 — по его регистрам
  `0x0000/0x0001`, и на ВСЕХ трёх адресах его таблицы (0x3c, 0x10, 0x2d),
  включая 0x10, по которому главный сенсор в своих чтениях отвечает
  нормально. Значит дело не в адресе и не в электрике.
- Отказ на фазе **send** (2 байта), при валидных `addr`, `len`, `buf` —
  то есть мимо всех трёх проверок `__mt_i2c_transfer`, которые дают
  `-EINVAL`. Источник глубже: либо `mt_i2c_do_transfer` через
  `i2c_set_speed`, либо путь расширения (`ext_flag` с
  `I2C_A_FILTER_MSG`, который выставляется на время поиска сенсора).
- Единственное заметное отличие вызова от рабочего драйвера: S5K5E8
  читает **1 байт** (`iReadRegI2C(cmd, 2, &b, 1, id)`), S5K4H8 — 2.
- Питание, пины и такт фронталки к этому моменту стоко-точные:
  `VCAMA 2.8 / VGP1 1.2 / VCAMIO 1.8`, `cam1_rst0 → cam1_rst1`,
  `ISP_MCLK1_EN(1)` вызывается (в `kdCISModulePowerOn` он выключён только
  для IMX135).

Следующий шаг для фронталки — не подбор, а инструментовка самого
`i2c-mtk.c`: напечатать, какая именно проверка возвращает `-EINVAL`
(добавить `dev_err` в каждую точку возврата в `__mt_i2c_transfer` и
`mt_i2c_do_transfer`). Это ответит за один прогон.

### Итог сессии 2026-08-27: рабочий образ `b18`, устройство в Android

Прошит и проверен `b18` (md5 `3f023ae75d205bfdda1c5d6c93166180`):
```
uname 4.9.188-m5c+   cpus 0-1 (hotplug)
Камера     Number of camera devices: 1   Facing: BACK
Wi-Fi      wlan0 d8:6c:02:ab:6f:3e, 20 точек в скане
Bluetooth  state: ON
Тач        mtk-tpd + mtk-tpd-kpd
```

**Осторожно:** пересборка того же состояния дерева (образ `ok`,
md5 `0d49912558a5ba150002c3c592777d95`) на устройстве НЕ загрузилась —
сработал forge deadman и телефон ушёл в TWRP, хотя `git diff` против
состояния `cam17` был пуст. Причина не выяснена; при следующем заходе
проверять загрузку каждого образа отдельно и не считать «то же дерево =
тот же результат». Восстановление делалось из TWRP:
`mount /data` → `dd if=/data/local/tmp/b18.img of=<by-name/boot>` →
readback → `reboot`.

История ветки содержит три подряд `Revert` вокруг правки
`kdSetI2CSpeed` — итоговое состояние совпадает с проверенным `cam17`
(`git diff 0e275aa8d` пуст), но выглядит грязно; при следующем заходе
имеет смысл собрать это в один коммит.

## 2026-08-27 — образ для тестера + разбор i2c-скорости до конца

**Готовый образ:** `device/meizu/m5c/artifacts/boot_m5c_49_20260827.img`
(md5 `4b4b9a35382273d6e3bf9e03222f0e2f`, 9 459 712 б, ядро 4.9.188-m5c+).
Проверен после холодной загрузки: Android грузится, задняя камера
перечисляется и снимает, Wi-Fi даёт 20 точек в скане, BT `state: ON`,
тач/звук/сенсоры на месте, CPU 0-2. Не работают: фронтальная камера,
сотовая связь (модем падает в exception), GPS-фикс, есть разрывы кадра.

### Источник `-EINVAL` установлен точно

`mt_i2c_do_transfer()` вызывает `i2c_set_speed()` на КАЖДОЙ транзакции и
возвращает `-EINVAL`, если та не смогла запрограммировать частоту
(`i2c-mtk.c`, ~строка 862). Скорость берётся из `client->timing`, куда
`kdSetI2CSpeed()` кладёт `imgsensor_info.i2c_speed` — а драйверы дерева
пишут туда килогерцы (200/400). Сообщение об ошибке не называло ни
запрошенной частоты, ни параметров шины — теперь называет.

Проверены на железе и **все три отвергнуты**:

| что пробовали | результат |
|---|---|
| оставить 400 как есть | фронталка нечитаема, задняя жива (успевает прочесть id до первого вызова) |
| кГц → Гц (400 → 400000) | `Failed to set the speed` уходит, но 400 кГц выше, чем настроена i2c0 (`def_speed = <0x64>`): пропадают ОБЕ камеры |
| кГц → Гц с клемпом 100 кГц | ошибка частоты уходит совсем (0 в логе), но отказов передач втрое больше (336 против 104) и задняя камера снова теряется |

Единственная конфигурация, где задняя камера работает, — та, в которой
`kdSetI2CSpeed()` вообще не переопределяет скорость, заданную в DT.
Функция сделана невмешивающейся (пишет в лог, что просил драйвер) —
подтверждено: `Number of camera devices: 1`, отказов снова 104.

Итог по фронталке: `-EINVAL` на её чтениях НЕ объясняется скоростью шины.
Остаётся выяснить, чем ещё отличается путь чтения S5K5E8 (он читает
**1 байт**, S5K4H8 — 2) и что происходит с контроллером между этими
транзакциями.

### Фронтальная камера: программная сторона исчерпана, версия сместилась на железо

Образ `c21` (md5 `fe06bcf5eaa27512037cdbfe64baf72e`): CMPDN фронталки
теперь явно отпускается (`pinctrl cam1_pnd0 ret=0` в логе). Сенсор
по-прежнему молчит, задняя камера цела (`Number of camera devices: 1`).

К этому моменту у фронталки стоко-точно ВСЁ, что можно задать софтом:

| параметр | значение | сверено с |
|---|---|---|
| Аналоговое питание | VCAMA 2.8 В | дизасм стокового `kdCISModulePowerOn` |
| Цифровое ядро | VGP1 1.2 В | там же (тип 4 = SUB_VCAMD) |
| IO | VCAMIO 1.8 В | там же |
| Reset | `cam1_rst0` → `cam1_rst1`, GPIO 11 | стоковый DTB: `cam1@0` output-low, `cam1@1` output-high |
| Power-down | `cam1_pnd0` (отпущен) | добавлено сверх стока |
| Такт | `ISP_MCLK1_EN(1)` | сток вызывает тот же гейт, MCLK2 не использует |
| Шина | i2c-0, клиент `camera_sub@3c` привязан | `client2 bound: i2c-0 dt-addr=0x3c` |
| Адрес | 0x78 первым в таблице | `s5k5e8yxmipiraw_Sensor.c:183` |

И при всём этом контроллер отвечает **`I2C_ACKERR`** на каждом адресе —
то есть на шине никто не отзывается. (`-22`, который мы видели, — это то,
как обёртка MTK маппит ACK-ошибку, а не программный отказ; ошибок
установки частоты в логе больше нет вовсе.)

HYPOTHESIS (проверяется только заменой модуля/шлейфа): сенсор этого
экземпляра неисправен. Косвенно поддерживается историей: до правки
тач-драйвера линия VGP1 — цифровое ядро фронталки — удерживалась на
2.8 В вместо 1.2 В при каждом включении панели.

Что осталось непроверенным программно и стоит попробовать при следующем
заходе, если появится второй аппарат: сравнить поведение на заведомо
...
### Модем: найдено расхождение ABI ядро ↔ демон (FACT)

Отвергнутый ioctl опознан: `cmd = 0x00004b01` = `_IO('K', 1)` =
**`CCCI_FS_IOCTL_GET_INDEX`** из legacy-драйвера
`drivers/misc/mediatek/dual_ccci/include/ccci_fs.h:36`. Пара к нему —
`CCCI_FS_IOCTL_SEND` `_IOR('K', 2, unsigned int)`.

Как устроена legacy-служба (`dual_ccci/ccci_fs_main.c`): демон **mmap**'ит
через `/dev/ccci_fs` разделяемый буфер (`fs_stream_buffer_t` × 5, по
16 КБ), ждёт индекс сообщения через `GET_INDEX` (kfifo) и отвечает через
`SEND`.

Как устроено у нас (`eccci1`): `ccci_fs` — обычный символьный порт, обмен
через `read`/`write` пакетами skb; `dev_char_mmap()` для него возвращает
`-EPERM`, ioctl-ов `'K'` нет вовсе.

То есть стоковый `ccci_fsd` из vendor-блобов написан под **другой**
ядерный драйвер, чем тот, что несёт LOS. Проверено, что это не регресс
4.9: в дереве 3.18 (LOS) `CONFIG_MTK_CCCI_EXT` не задан, `dual_ccci` не
собирается, а `eccci` там так же не знает ни `'K'`-ioctl, ни mmap на FS.
**Вывод: сотовая связь на LOS-порту m5c не работала никогда**, и exception
модема — следствие того, что ему некому отдать NVRAM.

Тот же блоб лежит и в vendor-дереве m681 (та же строка ошибки), то есть
это общая черта Meizu-блобов этого поколения, а не особенность m5c.

Пути решения, по стоимости:
1. **Реализовать legacy-совместимость в `eccci1`** — добавить порту
   `ccci_fs` mmap на разделяемый буфер и два ioctl (`GET_INDEX`, `SEND`),
   повторив механику `dual_ccci/ccci_fs_main.c` поверх существующего
...
## 2026-08-27 — разрывы: HWC и его фенсы проверены объективно

HWC пересобран (`hwcomposer/build_standalone.sh`, md5 `1f9c6619…` lib64 /
`bf9bc350…` lib32) и установлен на устройство. Процедурная заметка:
`stop surfaceflinger; start surfaceflinger` на живой системе оставляет её
в загрузочной анимации (`init.svc.bootanim=running`,
`mCurrentFocus=null`), и счётчики тогда снимаются с BootAnimation —
мерить надо после полной перезагрузки.

FACT (реальный UI, скролл списка настроек, RenderThread активен):
```
frames=6249  prepare_fail=0  trigger_fail=0  acq_timeout=0
ovl_frames=6245  gles_frames=4  promoted=10273  planes=2  fbt=0
acq: presig=10270  waited=32  nofence=13  wait_avg_us=6879  wait_max_us=18961
crc: runs=246  dirty_runs=0  last_dirty_mask=0x0
```
Чтение:
- `nofence=13` из 10 283 — SF практически всегда отдаёт фенс;
- `waited=32`, и ожидание настоящее (в среднем 6.9 мс, максимум 19 мс) —
  значит фенсы не пустышки: когда GPU не успевает, HWC действительно
  ждёт;
- `dirty_runs=0` за 246 проверок — после подачи буфера дисплею в него
  никто не пишет.

INFERENCE: ни «SF не отдаёт фенсы», ни «mali-фенс сигналит до конца
рендера», ни «производитель дописывает в отданный буфер» не
подтверждаются. Вместе с ранее закрытым ядром (латч в бланкинге, живой
trigger loop, IRQTime размазан) это выносит источник разрыва за пределы
и ядра, и нашего HWC — остаётся тракт до `queueBuffer`
(приложение/hwui/SurfaceFlinger).

Отдельно проверено и REJECTED (чтение ядра, правок не делалось):
«буфер уходит в оверлей раньше, чем защёлкнулся предыдущий конфиг».
`_config_ovl_input` (`primary_display.c`) выставляет
`cur_config_fence`/`subtractor_when_free`, а
`_ovl_fence_release_callback` освобождает буфер из колбэка завершения
конфиг-таска CMDQ, который сам сначала ждёт frame-done — то есть к
моменту релиза вытесненный буфер уже не сканируется.

## 2026-08-27 — mux: корень найден, это версия CCCI-порта в sysfs

`gsm0710muxd` **не использует** свой аргумент `-s`: в его `getopt`
optstring есть `s:`, но ветка `case 's'` уходит в конец цикла, не сохранив
`optarg`. Имя порта берётся исключительно из `ccci_get_node_name()`:
```
fd = open("/sys/kernel/ccci/version");
ver = (fd < 0) ? 1 : (первый байт - '0');
switch (ver) { case 1: case 2: case 3: case 5: name = "/dev/ttyC0"; break;
               default: return NULL; }
```
Наше ядро сообщает **6** (`ccci_port_ver = 6 /* ECCCI_FSM */`), поэтому
функция возвращает NULL, `serial.devicename` становится строкой
`"(null)"`, `open("(null)")` даёт ENOENT — и это разом объясняет и
`[(null)]` в дампе опций, и «Could not open serial device», и пустой
`/dev/radio`, и вечное `could not connect to /dev/radio/pttynoti` у RIL.
`ccci_mdinit` гейтит настройку узлов тем же условием (`==3 || ==5`).

Это тот же класс, что и с `ccci_fsd`: стоковый userspace — 3.18-эпохи.
Все 3.18-деревья сообщают версию 3, все 4.9 — 6.

Правка: `ccci_port_ver = 3`. Безопасность: значение читают только
`ccci_version_show` и `kcfg_setting_show`, ни одна ветка драйвера на нём
не завязана, `update_ccci_port_ver()` вызывается лишь из несобираемого
`dual_ccci`. Ожидаемый побочный эффект — `kcfg_setting` показывает
`[ccci_drv_ver]:V1` вместо V2, как на стоке.
Подтверждено на устройстве до правки: `cat /sys/kernel/ccci/version` = 6.

Заметка на будущее: два первых билда упали на самом комментарии к этой
...
## 2026-08-27 (вечер) — разрывы: вердикт «hwui partial updates» СНЯТ вечерним A/B

A/B на пересобранном libhwui (ручка подтверждённо работает: `Swap
behavior 0/1` по `debug.hwui.partial_updates`): hard-счётчик shear-пробы
ИНВАРИАНТЕН к ручке — 39/1152 (до фикса, ON), 37/1182 (OFF), 39/1066
(ON). При Discard двухмоментный кадр от hwui невозможен → HARD-кадры
были алиасингом повторяющихся элементов списка (пары одиночных строк с
шагом ровно 312 px, Δd ровно 20 — во всех трёх прогонах), а не разрывами.
Гипотеза 6 REJECTED моей же контрольной пробой; фикс оставлен как
hardening. Причина стежка на стекле снова ОТКРЫТА, при этом все
замеренные слои (латч ядра, фенсы, CRC, продюсер) говорят «в буфере
разрыва нет». Следующий шаг — глаза на текущей сборке; затем
maxplanes=1 против underflow. Правило чтения shear-карт: одиночные
строки-зоны игнорировать, зона = ≥2 соседних пробных строк. Попутно:
